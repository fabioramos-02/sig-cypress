...
describe('Sistema Integrado de Gestão para Fundações de Amparo a Pesquisas - Proposta Média', () => {
    ...
    it('Realiza login no sistema e submete uma proposta completa para o Edital 37', () => {
        acessarPaginaDeEditais();
        selecionarEdital(103);

        cy.get('[data-cy="criar-proposta"]').click(); // Cria uma nova proposta

        preencherInformacoesIniciais();
        preencherInformacoesComplementares(); // Preenche as informações complementares da proposta
    ...
    const preencherInformacoesComplementares = () => {
        ...
        // Preencher data da realização do evento com o dia de hoje (caso necessário, adicione esse passo aqui)

        // Marca a opção "Média de Faturamento" clicando no item correspondente
        cy.get('[data-cy="formularioPropostaInformacaoComplementar.pergunta-24-item-media-faturament"]').click(); // Clica na opção "Média de Faturamento"
    }
